...
public class A7 {
    ...
    public static List<Node> dijkstra(Node v, Node end) {
        ...
        HashMap<Node, Item> S= new HashMap<>();

        F.insert(v, 0);
        S.put(v, new Item(0, null));

        while (F.size != 0) {
            Node f= F.poll();
            List<Edge> edge= f.exits();
            if (f == end) return path(S, end);

            for (Edge e : edge) {
                Node w= e.other(f);
                int wLength= S.get(f).dist + e.length;

                if (!S.containsKey(w)) {
                    S.put(w, new Item(wLength, f));
                    F.insert(w, wLength);
                } else if (wLength < S.get(w).dist) {
                    S.get(w).dist= wLength;
                    S.get(w).bkptr= f;
